...
-- Folder Config
local CONFIG_FOLDER = "RTaODevConfig"
...
local function cleanupSpeed()
    ...
    if guiToggle then guiToggle.Text = "▶ Turn On" end
end

local function createGUI()
    if gui then gui:Destroy() gui = nil end
    gui = Instance.new("ScreenGui")
    gui.Name = "RTaOSpeedGrappleGUI"
    ...
    guiToggle.Text = enabled and "⏸ Turn Off" or "▶ Turn On"
...
local function sendWebhook(name, btype, price, jobId, placeId, mutation, players)
    ...
    local embed = {
        title = "🧠 New Pet Found - " .. (now or "Unknown Time"),
...
local function cleanupTweenToBase()
    ...
    if playerGui then
        local gui = playerGui:FindFirstChild("RTaOWalkGui")
        if gui then
            gui:Destroy()
        end
    end
